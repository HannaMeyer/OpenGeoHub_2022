...
legend("topleft",pch=1,legend=c("training","validation"),bty="n",col=c("black","blue"),pt.cex=c(0.5,0.1))
```
...
# estimate performance via NNDM cross-validation:
custom <- rsmp("custom")
train_sets <- NNDM_cv$indx_train # derived from CAST package
test_sets <- NNDM_cv$indx_test
rsmp_cv <- custom$instantiate(task, train_sets, test_sets)

rr <- resample(task, lrn, rsmp_cv)
 

## predict:
prediction <- predict(predictors,lrn$model)
...
AOA <- aoa(predictors,
           ...
           CVtest = rsmp_cv$instance$test,
           CVtrain = rsmp_cv$instance$train)
